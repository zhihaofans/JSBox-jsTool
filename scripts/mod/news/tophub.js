...
function getWeb(authkey) {
    ...
    $http.get({
        ...
        if (httpData) {
            ...
            var resultList = [];
            $("div.weui-panel__bd").find("a.weui-media-box.weui-media-box_appmsg").each(function (i, elem) {
                var sub = $(this).find("p.weui-media-box__desc").text();
                sub = sub.substring(1, sub.indexOf(" ", 1));
                const media_item = new ResultItem(
                    $(this).find("div.weui-media-box__bd > h4.weui-media-box__title").text(),
                    sub,
                    $(this).attr("href")
                );
            ...
            if (resultList.length > 0) {
                ...
                $ui.push({
                    ...
                    views: [{
                        type: "list",
                        props: {
                            data: resultList.map(i => `[${i.subtitle}]${i.title}`)
                        },
                        layout: $layout.fill,
                        events: {
