...
function sendLiveGiftList(liveData, giftList, index = 0) {
  ...
  if (giftList.length > 0) {
    ...
    const url = `${
      _BILIURL.LIVE_GIFT_SEND
    }?access_key=${_USER.getAccessKey()}&ruid=${liveData.target_id}&biz_id=${
      liveData.room_id
    }&bag_id=${thisGift.bagId}&gift_id=${thisGift.giftId}&gift_num=${
      thisGift.number
    }`;
    if (index === 0) {
    ...
    $http
      ...
      .then(function (resp) {
        ...
        if (data.code === 0) {
          const resultData = data.data;
          $console.info(
            `第${index + 1}组礼物(${thisGift.gift_name}${thisGift.number}个)：${
              resultData.send_tips
            }`
          );
