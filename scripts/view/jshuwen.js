    ...
    $http.get({
        ...
        if (resp.error) {
            ...
            if (data) {
                ...
                let linkList = [];
                $("div.aricle_item_info > div.title > a").each(function (i, elem) {
                    const thisLink = $(elem).attr("href").startsWith("/d/") ? `https://www.jishuwen.com${$(elem).attr("href")}` : $(elem).attr("href");
                            ...
                            menu: {
                                ...
                                items: [{
                                    ...
                                    handler: (sender, indexPath) => {
                                        const thisItem = linkList[indexPath.row];
                                        appScheme.safariAddReadingItem(thisItem.link, thisItem.title, thisItem.title);
                                    }
                                }, {
                                    title: "预览模式",
                                    symbol: "book",
