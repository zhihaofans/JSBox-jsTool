        ...
        switch (thisGift.gift_id) {
          case 6:
            if (needExp >= 10) {
              ...
              if (giftNum > 0) {
                giftList.push(
                  new GiftData(thisGift.gift_id, thisGift.bag_id, giftNum)
                );
      ...
      handler: function(resp) {
        ...
        if (giftResult.code == 0) {
          ...
          const giftTitleList = giftList.map(
            gift => `${gift.gift_name}（${gift.corner_mark}）${gift.gift_num}个`
          );
          $ui.loading(false);
          if (giftList.length) {
                      ...
                      items: [
                        {
                          ...
                          handler: (sender, indexPath) => {
                            ...
                            $app.openURL(
                              _BILIURL.LIVE_WEB_ROOM + liveData.room_id
                            );
                          }
                        },
                        {
              ...
              actions: [
                {
                  title: "换硬币",
                  disabled: !(
                    walletData.silver_2_coin_left > 0 && walletData.status > 0
                  ),
                  handler: function() {
                    if (
                      ...
                      $http.post({
                        ...
                        header: {
                          "User-Agent":
                            "bili-universal/9290 CFNetwork/1125.2 Darwin/19.4.0 os/ios model/iPhone 11 mobi_app/iphone osVer/13.4 network/2",
                          "Content-Type": "application/x-www-form-urlencoded"
                        },
                        body: {
  ...
  if (_userData.access_key) {
    $http
      .get({
        ...
        header: {
          "User-Agent":
            "bili-universal/9290 CFNetwork/1125.2 Darwin/19.4.0 os/ios model/iPhone 11 mobi_app/iphone osVer/13.4.1 network/1"
        }
      })
      .then(function(resp) {
        var data = resp.data;
